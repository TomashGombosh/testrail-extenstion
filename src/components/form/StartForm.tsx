...
const StartForm = () => {
  ...
  const [isDisplaySetProjects] = useState<boolean>(localStorage.getItem(TEST_RAIL_PROJECT_IDS_ATTRIBUTE) === null);
  ...
  return (
    <Grid container direction="row" alignItems="flex-start" alignContent="flex-start" className="form">
      ...
      {!isLoading && (
        <>
          {isDisplaySetToken && (
            <Grid item className="start-screen-form-item">
          ...
          {!isDisplaySetToken && !isDisplaySetProjects && (
            <Grid item className="start-screen-form-item">
